...
use super::{identity_babybear, identity_p254, join, lift, prove::zkr, MerkleGroup, Prover};
use crate::{
...
#[test]
fn test_recursion_lift_join_identity_babybear_e2e() {
    // Prove the base case
    let (session, segments) = generate_busy_loop_segments("poseidon2");
    ...
    // Check on stark-to-snark
    // let snark_receipt =
    identity_babybear(&rollup).expect("Running prover failed");
...
#[test]
fn test_recursion_identity_poseidon2() {
    ...
    let (_, control_id_poseidon2) = zkr::identity("poseidon2").unwrap();

    let opts = ProverOpts::succinct()
        .with_hashfn("poseidon2".to_string())
        .with_control_ids(vec![control_id_poseidon2]);

    let mut prover = Prover::new_identity(&default_receipt, opts.clone()).unwrap();
    let poseidon2_recursion_receipt = prover.run().unwrap();
    let mut out_stream = VecDeque::<u32>::new();
    out_stream.extend(poseidon2_recursion_receipt.output.iter());

    // Include an inclusion proof for control_id to allow verification against a root.
    let hashfn = opts.hash_suite().unwrap().hashfn;
    let poseidon2_control_tree = MerkleGroup::new(opts.control_ids.clone()).unwrap();
    let poseidon2_control_inclusion_proof = poseidon2_control_tree
        .get_proof(prover.control_id(), hashfn.as_ref())
        .unwrap();
    let poseidon2_control_root = poseidon2_control_tree.calc_root(hashfn.as_ref());
    let params = SuccinctReceiptVerifierParameters {
        control_root: poseidon2_control_root,
        inner_control_root: Some(ALLOWED_CONTROL_ROOT),
        proof_system_info: PROOF_SYSTEM_INFO,
        circuit_info: CircuitImpl::CIRCUIT_INFO,
    };
    let poseidon2_receipt = SuccinctReceipt {
        seal: poseidon2_recursion_receipt.seal,
        hashfn: opts.hashfn,
        control_id: control_id_poseidon2,
        control_inclusion_proof: poseidon2_control_inclusion_proof,
